...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState, costFn = lambda x: 1, warn=True, visualize=True):
        ...
        self.goalState_found = {(1,1): False, (1,top): False , (right, 1): False, (right, top): False}
        self.numberOfCornersFound = (False, False, False, False)
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        return (self.startingPosition, self.numberOfCornersFound)
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                if (nextx,nexty) in self.corners:
                    index_of_corner = self.corners.index((nextx,nexty))
                    if index_of_corner == 0:
                        a = True
                    elif index_of_corner == 1:
                        b = True
                    elif index_of_corner == 2:
                        c = True
                    else:
                        d = True
                successors.append(((nextState, (a, b, c, d)), action, cost))
...
def cornersHeuristic(state, problem):
    ...
    for x in state[1]:
        if not x:
            unvisited_corners.append(corners[temp])
        temp += 1

    # calculate manhattan distance from closest corners to all closest unvisited corners
    heuristic_value = 0
    postition_pac = state[0]
    while unvisited_corners != []:
        distances = []
        for val in unvisited_corners:
            distances.append(manhattanDistance(postition_pac, val))
        min_distance = min(distances)
        heuristic_value += min_distance
        min_distance_index = distances.index(min_distance)
        postition_pac = unvisited_corners[min_distance_index]
        unvisited_corners.remove(unvisited_corners[min_distance_index])
